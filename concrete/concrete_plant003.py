...
class ConcretePlant003(SacPlant):
    ...
    def __init__(self, h = 15/60/24, volume = 12, flow = 24, rho = 1/24, pgain = 100., amplitudePeriodicDv = 1.0, SvNh4 = 3.0, timeIntegral = 1/24/4, thresholdDo = 1.5, maxDo = 3.0, minDo = 0.0, weightOnMv = 0.0):
        ...
        self.idxSO2 = self.asmVarNames.index("S_O2")
        self.idxNH4 = self.asmVarNames.index("S_NH4")
        self.SvNh4 = SvNh4
    ...
    def f(self, t, x, Dv, Mv):
        ...
        dxAsmdtAsm= np.array(self.getBiologicalProcess(*xAsm)) # (nAsm,)         
    ...
    def generateDv(self):
        ...
        xInflow = np.array(self.getDefaultInflow()) # (nAsm,)
