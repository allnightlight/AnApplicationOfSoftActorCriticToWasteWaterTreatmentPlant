...
class Work018(WorkTemplate):
    '''

    2020-12-15

    To examine the effect of weightOnMv
    ...
    def generateBuildParameter(self):
        ...
        for _ in range(self.nAgent):
            ...
            weighOnMv = float(np.random.choice([0.0, 0.1, 0.5, 0.9, 1.0]))
            
            nQfunctionRedundancy = int(np.random.choice([1,]))
            
            nBatch = 2**5
            nStepGradient = 1
            learningRateForUpdatePolicy = float(np.random.choice([1e-3,]))/nStepGradient
            learningRateForUpdateActionValueFunction = float(np.random.choice([1e-2]))/nStepGradient            
            learningRateForUpdateStateValueFunction = float(np.random.choice([1e-2,]))/nStepGradient
